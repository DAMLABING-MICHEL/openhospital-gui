...
public class IoOperations {
	...
	public Disease getDiseaseByCode(int code) throws OHException {
		...
		try{
			...
			while (resultSet.next()) {
				disease = parseDiseaseRecord(resultSet);
			}
	...
	/**
	 * Returns a {@link Disease} object given its code.
	 * @param code String, the {@link Disease} code.
	 * @return {@link Disease} object.
	 * @throws OHException
     */
	public Disease getDiseaseByCode(String code) throws OHException {
		Disease disease = null;
		DbQueryLogger dbQuery = new DbQueryLogger();
		try{
			String query = "SELECT * FROM DISEASE JOIN DISEASETYPE ON DIS_DCL_ID_A = DCL_ID_A WHERE DIS_ID_A = ?";
	...
	/**
	 * Parse a database row into a {@link Disease} object.
	 * @param resultSet {@link ResultSet} returned from the db.
	 * @return a {@link Disease} object.
	 * @throws SQLException
     */
	public static Disease parseDiseaseRecord(ResultSet resultSet) throws SQLException {
		Disease disease = new Disease(resultSet.getString("DIS_ID_A"),
				...
				resultSet.getInt("DIS_MIN_AGE"),
				resultSet.getInt("DIS_MAX_AGE"),
				resultSet.getBoolean("DIS_MALE"),
				resultSet.getBoolean("DIS_FEMALE"),
				resultSet.getBoolean("DIS_IS_CHRONIC"),
	...
	/**
	 * Filter diseases based on patient information.
	 * @param patGender char, patient gender.
	 * @param patMonths int, patient months.
	 * @return ArrayList of {@link Disease}.
	 * @throws OHException
     */
	public  ArrayList<Disease> getOpdDiseases(char patGender, int patMonths, String typeCode, boolean chronic) throws OHException {
		ArrayList<Object> params = new ArrayList<Object>(3);
		String query = "SELECT * FROM DISEASE JOIN DISEASETYPE ON DIS_DCL_ID_A = DCL_ID_A WHERE" +
				" (DIS_OPD_INCLUDE=1 OR DIS_OPD_INCLUDE IS NULL)";
		query += patGender == 'F' ? " AND DIS_FEMALE = 1" : " AND DIS_MALE = 1";
		query += " AND (DIS_MIN_AGE = 0 OR DIS_MIN_AGE <= ?)" +
				" AND (DIS_MAX_AGE = 0 OR DIS_MAX_AGE >= ?)";
		params.add(patMonths);
		params.add(patMonths);
		if (typeCode != null) {
			query += " AND DCL_ID_A = ?";
			params.add(typeCode);
		...
		if (chronic)
			query += " AND DIS_IS_CHRONIC = 1";
		query += " ORDER BY DIS_DESC;";
		...
		try{
			...
			diseases = new ArrayList<Disease>(resultSet.getFetchSize());
			while (resultSet.next()) {
				diseases.add(parseDiseaseRecord(resultSet));
			}
		} catch (SQLException e) {
			throw new OHException(MessageBundle.getMessage("angal.sql.problemsoccurredwiththesqlistruction"), e);
		} finally{
	...
	public boolean newDisease(Disease disease) throws OHException{
		...
		try{
			...
			parameters.add(disease.getMinimumMonths());
			parameters.add(disease.getMaximumMonths());
			parameters.add(disease.isFemale());
			parameters.add(disease.isMale());

			String query = "insert into DISEASE (DIS_ID_A, DIS_DCL_ID_A, DIS_DESC, DIS_OPD_INCLUDE, DIS_IPD_IN_INCLUDE, DIS_IPD_OUT_INCLUDE, DIS_MIN_AGE, DIS_MAX_AGE, DIS_FEMALE, DIS_MALE) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
	...
	public boolean updateDisease(Disease disease) throws OHException {
		...
		try{
			...
			String query = "UPDATE DISEASE SET DIS_DCL_ID_A = ?, DIS_DESC = ?, DIS_OPD_INCLUDE = ?, DIS_IPD_IN_INCLUDE = ?, DIS_IPD_OUT_INCLUDE = ?, DIS_MIN_AGE = ?, DIS_MAX_AGE = ?, DIS_FEMALE = ?, DIS_MALE = ?, " +
