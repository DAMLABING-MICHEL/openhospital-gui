...
public class MedicalBrowsingManager {
	 ...
	 */
	public ArrayList<Medical> getMedicalsSortedByName() {
		try {
			return ioOperations.getMedicals(true);
		} catch (OHException e) {
			JOptionPane.showMessageDialog(null, e.getMessage());
			return null;
		}
	 ...
	 * @return all the medicals.
	 */
	public ArrayList<Medical> getMedicalsSortedByCode() {
		try {
			return ioOperations.getMedicals(false);
		} catch (OHException e) {
			JOptionPane.showMessageDialog(null, e.getMessage());
			return null;
		}
	 ...
	 */
	public ArrayList<Medical> getMedicals(String type, boolean nameSorted) {
		try {
	...
	/**
	 * Finds a Medical searching by its code or description.
	 * @param hint String representing the starting hint for either the code or the description of the {@link Medical}.
	 * @return ArrayList of {@link Medical} object.
     */
	public ArrayList<Medical> findMedicalsByCodeOrDescription(String hint) {
		try {
			return ioOperations.findMedicalsByCodeOrDescription(hint);
		} catch (OHException e) {
			JOptionPane.showMessageDialog(null, e.getMessage());
			return null;
		}
	...
	public boolean newMedical(Medical medical) {
		...
		try {
			...
			boolean medicalCodeExists = ioOperations.medicalCodeExists(medical);
			if (medicalExists) {
				JOptionPane.showMessageDialog(null, MessageBundle.getMessage("angal.medicals.thetypemedicalyouinsertedwasalreadyinuse"));
				return false;
			} else if (medicalCodeExists) {
				JOptionPane.showMessageDialog(null, MessageBundle.getMessage("angal.medicals.themedicalcodeyouinsertedwasalreadyinuse"));
				return false;
