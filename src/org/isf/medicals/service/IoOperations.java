...
public class IoOperations {
	 ...
	 * Gets all stored {@link Medical}s.
	 * @return all the stored medicals.
	 * @param nameSorted - if <code>true</code> return the list in alphatecial order, by code otherwise
	 * @throws OHException if an error occurs retrieving the stored medicals.
	 */
	public ArrayList<Medical> getMedicals(boolean nameSorted) throws OHException {
		return getMedicals(null, nameSorted);
	}
	 ...
	 * @throws OHException if an error occurs retrieving the stored medicals.
	 */
	public ArrayList<Medical> getMedicals(String type, boolean nameSorted) throws OHException {
		...
		if (nameSorted) query.append("order by MDSR_DESC");
		else query.append("order by length(MDSR_CODE), MDSR_CODE, MDSR_DESC");
	...
	public ArrayList<Medical> getMedicals(String description, String type, boolean expiring) throws OHException {
		...
		try{
			...
			medicals = new ArrayList<Medical>(resultSet.getFetchSize());
			while (resultSet.next()) {
				medicals.add(parseMedicalRecord(resultSet));
			}
		}catch (SQLException e) {
			throw new OHException(MessageBundle.getMessage("angal.sql.problemsoccurredwiththesqlistruction"), e);
		} finally{
	...
	/**
	 * Return a list of {@link Medical} object searching by code or description.
	 * @param hint String, an hint for the starting of either the code or the name of the {@link Medical}.
	 * @return ArrayList of {@link Medical} object.
	 * @throws OHException
     */
	public ArrayList<Medical> findMedicalsByCodeOrDescription(String hint) throws OHException {
		ArrayList<Medical> medicals = new ArrayList<Medical>();
		String query = "SELECT * FROM MEDICALDSR" +
				" JOIN MEDICALDSRTYPE ON MDSR_MDSRT_ID_A = MDSRT_ID_A" +
				" WHERE MDSR_CODE LIKE ? OR MDSR_DESC LIKE ? ORDER BY MDSR_DESC";
	...
	/**
	 * Converts a databse row into a {@link Medical} object.
	 * @param resultSet {#link ResultSet} returned from the database.
	 * @return a {@link Medical} object.
	 * @throws SQLException
     */
	public Medical parseMedicalRecord(ResultSet resultSet) throws SQLException {
		return new Medical(
				resultSet.getInt("MDSR_ID"),
				new MedicalType(
						resultSet.getString("MDSR_MDSRT_ID_A"),
	...
	/**
	 * Checks if the specified code for the {@link Medical} exists or not.
	 * @param medical - the medical to check.
	 * @return <code>true</code> if exists <code>false</code> otherwise.
	 * @throws OHException if an error occurs during the check.
	 */
	public boolean medicalCodeExists(Medical medical) throws OHException
	{
		boolean result = false;
		List<Object> parameters = new ArrayList<Object>(1);
		parameters.add(medical.getProd_code());
		String query = "SELECT MDSR_ID FROM MEDICALDSR WHERE MDSR_CODE = ?";		
		DbQueryLogger dbQuery = new DbQueryLogger();
