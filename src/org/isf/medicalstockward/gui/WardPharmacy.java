...
import org.isf.stat.manager.GenericReportPharmaceuticalStockWard;
...
public class WardPharmacy extends ModalJFrame implements 
	...
	WardPharmacyDispense.MovementWardListeners, 
	WardPharmacyRectify.MovementWardListeners,
	WardPharmacyReturn.MovementWardListeners {
	...
	public void prescriptionInserted(AWTEvent e) {
		jTablePrescription.setModel(new PrescriptionModel());
		//jTabbedPaneWard.setSelectedComponent(jScrollPanePrescription);
	}
	...
	public void missingInserted(AWTEvent e) {
		jTableMissing.setModel(new MissingModel());
	}
	...
	private JScrollPane jScrollPanePrescription;
	...
	private JTable jTableMissing;
	private JTable jTablePrescription;
	private JScrollPane jScrollPaneIncomes;
	private JScrollPane jScrollPaneMissing;
	...
	private DefaultTableModel modelMissing;
	...
	private String[] columsIncomes = { 
			MessageBundle.getMessage("angal.medicalstockward.date"), 
	...
	private String[] columsMissing = { 
			MessageBundle.getMessage("angal.medicalstockward.date"),
			MessageBundle.getMessage("angal.medicalstockward.medical"),
			MessageBundle.getMessage("angal.medicalstockward.quantity")
	};
	private boolean[] columsResizableMissing = { false, true, false };
	private int[] columWidthMissing = { 80, 320, 200 };
	...
	private String[] columsPrescription = { "Date", "Bill ID", "PID", MessageBundle.getMessage("angal.medicalstockward.patient"), "Amount", "Bill", "Dispense"};
	private boolean[] columsResizablePrescription = { false, false, false, true, false, false, false};
	private int[] columWidthPrescription = { 150, 100, 100, 300, 100, 80, 80 };
	...
	private ArrayList<MissingMedical> wardMissing;
	private ArrayList<MedicalWardPrescription> wardPrescription;
	...
	public WardPharmacy() {
		...
		addWindowListener(new WindowAdapter() {
			...
			public void windowClosing(WindowEvent e) {
				...
				if (wardMissing != null)
					wardMissing.clear();
				if (wardPrescription != null)
					wardPrescription.clear();
				dispose();
			}
		});
	}
	...
	private JPanel getJPanelButtons() {
		if (jPanelButtons == null) {
			...
			jPanelButtons.add(getJButtonReturn());
			if (editAllowed)
	...
	private JButton getJButtonStockCard() {
		if (jButtonStockCard == null) {
			jButtonStockCard = new JButton(MessageBundle.getMessage("StockCard"));
			...
			jButtonStockCard.addActionListener(new ActionListener(){
				public void actionPerformed(ActionEvent event){
					...
					else if (jTabbedPaneWard.getSelectedIndex() == 3) {
						if (jTableMissing.getSelectedRow() >= 0) {
							MissingMedical missingMedical = (MissingMedical) ((jTableMissing.getModel()).getValueAt(jTableMissing.getSelectedRow(), -1));
							medical = missingMedical.getMedical();
						}
					}
	...
	private JButton getJButtonStockLedger() {
		if (jButtonStockLedger == null) {
			jButtonStockLedger = new JButton(MessageBundle.getMessage("StockLedger"));
	...
	private JButton getJButtonNew() {
		if (jButtonNew == null) {
			...
			jButtonNew.setText(MessageBundle.getMessage("angal.medicalstockward.new")); //$NON-NLS-1$
	...
	private JButton getJButtonReturn() {
		if (jButtonReturn == null) {
			jButtonReturn = new JButton();
			jButtonReturn.setText("Return"); //$NON-NLS-1$
			jButtonReturn.setMnemonic(KeyEvent.VK_R);
			jButtonReturn.setBackground(Color.PINK);
			jButtonReturn.setVisible(false);
			jButtonReturn.addActionListener(new ActionListener() {

				public void actionPerformed(ActionEvent e) {
					WardPharmacyReturn editor = new WardPharmacyReturn(WardPharmacy.this, wardSelected, wardDrugs);
					editor.addMovementWardListener(WardPharmacy.this);
					editor.setVisible(true);
					/* } */
				}
	...
	private JButton getJButtonEdit() {
		if (jButtonEdit == null) {
			...
			jButtonEdit.setText(MessageBundle.getMessage("angal.medicalstockward.edit")); //$NON-NLS-1$
	...
	private JButton getJButtonClose() {
		if (jButtonClose == null) {
			...
			jButtonClose.setText(MessageBundle.getMessage("angal.medicalstockward.close")); //$NON-NLS-1$
	 ...
	 * 
	 * if (jTableOutcomes.getSelectedRow() < 0 ||
	 * !jScrollPaneOutcomes.isShowing()) { JOptionPane.showMessageDialog(WardPharmacy.this,
	 * MessageBundle
	...
	private JScrollPane getJScrollPaneMissing() {
		if (jScrollPaneMissing == null) {
			jScrollPaneMissing = new JScrollPane();
			jScrollPaneMissing.setViewportView(getJTableMissing());
		}
		return jScrollPaneMissing;
	}
	...
	private JTable getJTableMissing() {
		if (jTableMissing == null) {
			modelMissing = new MissingModel();
			jTableMissing = new JTable(modelMissing);
			for (int i = 0; i < columWidthMissing.length; i++) {
				jTableMissing.getColumnModel().getColumn(i).setMinWidth(columWidthMissing[i]);
				if (!columsResizableMissing[i])
					jTableMissing.getColumnModel().getColumn(i).setMaxWidth(columWidthMissing[i]);
			}
			jTableMissing.setAutoCreateColumnsFromModel(false);
		}
		return jTableMissing;
	}
	...
	private JComboBox getJComboBoxMedicals() {
		...
		ArrayList<Medical> medicals = medicalManager.getMedicalsSortedByCode();
	...
	private JTabbedPane getJTabbedPaneWard() {
		if (jTabbedPaneWard == null) {
			...
			jTabbedPaneWard.addTab("Missing", getJScrollPaneMissing());
			jTabbedPaneWard.addTab("Prescriptions", getJScrollPanePrescription());
		}
	...
	private JScrollPane getJScrollPanePrescription() {
		if (jScrollPanePrescription == null) { 
			jScrollPanePrescription = new JScrollPane();
			jScrollPanePrescription.setViewportView(getJTablePrescription());
		}
		return jScrollPanePrescription;
	}
	
	 JTable getJTablePrescription() {
		if (jTablePrescription == null) {
			modelPrescription = new PrescriptionModel();
			jTablePrescription = new JTable(modelPrescription);
			jTablePrescription.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
			for (int i = 0; i < columWidthPrescription.length; i++) {
				jTablePrescription.getColumnModel().getColumn(i).setPreferredWidth(columWidthPrescription[i]);
				if (!columsResizablePrescription[i])
					jTablePrescription.getColumnModel().getColumn(i).setMaxWidth(columWidthPrescription[i]);
			}
			jTablePrescription.getColumnModel().getColumn(1).setCellRenderer(new CenterBoldTableCellRenderer());
			jTablePrescription.getColumnModel().getColumn(2).setCellRenderer(new CenterBoldTableCellRenderer());
			jTablePrescription.getColumnModel().getColumn(5).setCellRenderer(new CenterBoldTableCellRenderer());
			jTablePrescription.getColumnModel().getColumn(6).setCellRenderer(new CenterBoldTableCellRenderer());
			jTablePrescription.setAutoCreateColumnsFromModel(false);
			jTablePrescription.addMouseListener(new MouseListener() {
				...
				public void mouseClicked(MouseEvent e) {
					if (e.getClickCount() == 2 && !e.isConsumed()) {
						...
						MedicalWardPrescription prescription = (MedicalWardPrescription) jTablePrescription.getValueAt(row, -1);
						WardPharmacyDispense dispense = new WardPharmacyDispense(WardPharmacy.this, prescription);
						dispense.addMovementWardListener(WardPharmacy.this);
						dispense.setVisible(true);
					}
				}
			});
		}
		return jTablePrescription;
	}
	...
	class MissingModel extends DefaultTableModel {

		/**
		 * 
		 */
		private static final long serialVersionUID = 1L;
		//private ArrayList<Movement> wardMissing; --> Global

		public MissingModel() {
			wardMissing = new ArrayList<MissingMedical>();
			wardMissing = wardManager.getMissingMedicals(wardSelected.getCode(), dateFrom, dateTo);
		}

		public int getRowCount() {
			if (wardMissing == null)
				return 0;
			return wardMissing.size();
		}

		public Object getValueAt(int r, int c) {
			MissingMedical med = wardMissing.get(r);
			if (c == -1) {
				return med;
			}
			if (c == 0) {
				return formatDate(med.getDate());
			}
			if (c == 1) {
				return med.getMedical();
			}
			if (c == 2) {
				return med.getQty();
			}
		...
		public String getColumnName(int c) {
			return columsMissing[c];
		}

		public int getColumnCount() {
			return columsMissing.length;
		}
		...
		public Object getValueAt(int r, int c) {
			...
			if (c == 3) {
				...
				if (pcsPerPck != 0) {
					...
					return "" + packets + " " + MessageBundle.getMessage("angal.medicalstockward.packets"); //$NON-NLS-1$;
				} else {
	...
	class PrescriptionModel extends DefaultTableModel {
		...
		public PrescriptionModel() {
			wardPrescription = wardManager.getPrescriptions(wardSelected, dateFrom, dateTo);
			Collections.reverse(wardPrescription);
		}
		...
		public int getRowCount() {
			if (wardPrescription == null)
				return 0;
			return wardPrescription.size();
		}

		public Object getValueAt(int r, int c) {
			MedicalWardPrescription mov = wardPrescription.get(r);
			Bill bill = mov.getBill();
			if (c == -1) {
				return mov;
			}
			if (c == 0) {
			...
			if (c == 2) {
				return mov.getPatient().getCode();
			}
			if (c == 3) {
				return bill.getPatName();
			}
			if (c == 4) {
			...
			if (c == 5) {
				...
				if (status.equals("D")) return "Cancelled";
				if (status.equals("C")) return "Paid";
				if (status.equals("L")) return "Pay Later";
				if (status.equals("X")) return "Unclaimed";
				else return "";
			}
			if (c == 6) {
				int status = mov.getStatus();
				if (status == 0) {
					return "Wait";
				} else if (status == 1) {
					return "Yes";
		...
		public int getColumnCount() {
			return columsPrescription.length;
		}
	...
	class DrugsModel extends DefaultTableModel {
		...
		public Object getValueAt(int r, int c) {
			...
			if (c == 0) {
				return wardDrug.getMedical().getDescriptionWithCode();
			}
			if (c == 1) {
	...
	private JButton getJRectifyButton() {
		if (jRectifyButton == null) {
			jRectifyButton = new JButton("Rectify");
	...
	private JButton getPrintTableButton() {
		if (jPrintTableButton == null) {
			jPrintTableButton = new JButton("Report");
...
//						new PrintManager("WardPharmacyDrugs", wardManager.convertWardDrugs(wardSelected, wardDrugs), 0);
						...
						if (options.indexOf(option) == i) {
								...
								try {
									BusyState.setBusyState(WardPharmacy.this, true);
									new GenericReportPharmaceuticalStockWard(null, "PharmaceuticalStockWard", wardSelected);
									return;
								} finally {
									BusyState.setBusyState(WardPharmacy.this, false);
								}
						...
						if (options.indexOf(option) == ++i) {
					        ...
					        if (r == JOptionPane.OK_OPTION) {
								...
								try {
									BusyState.setBusyState(WardPharmacy.this, true);
									new GenericReportPharmaceuticalStockWard(dateChooser.getDate(), "PharmaceuticalStockWard", wardSelected);
									return;
								} finally {
									BusyState.setBusyState(WardPharmacy.this, false);
								}
						...
						new PrintManager("WardPharmacyMissing", wardManager.convertWardMissing(wardSelected, wardMissing), 0);
					} 
				}
			});
		}
		...
		if (jExportToExcelButton == null) {
			...
			jExportToExcelButton.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent arg0) {
					...
					StringBuilder fileName = new StringBuilder(wardSelected.getDescription()).append("_");
					if (index == 0) 
					{
						fileName.append(MessageBundle.getMessage("angal.medicalstockward.outcomes"));
					} 
					else if (index == 1) 
					{
						fileName.append(MessageBundle.getMessage("angal.medicalstockward.incomings"));
					} 
					else if (index == 2) 
					{
						fileName.append(MessageBundle.getMessage("angal.medicalstockward.drugs"));
					} 
					else if (index == 3)
					{
						fileName.append("Missing");
					} 
					else if (index == 4) 
					{
						fileName.append("Prescriptions");
					}
					...
					fcExcel.setSelectedFile(new File(fileName.toString()));
					...
					if(iRetVal == JFileChooser.APPROVE_OPTION) {
						try {
							...
							if (index == 0) {
								...
								xlsExport.exportTableToExcel(jTableMissing, exportFile);
							} else if (index == 4) {
								xlsExport.exportTableToExcel(jTablePrescription, exportFile);
							}
