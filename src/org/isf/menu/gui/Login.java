...
import org.isf.utils.db.MD5Encrypter;
...
public class Login extends JDialog implements ActionListener, KeyListener {
	...
	// elementi per la comunicazione con mainmenu
	...
	private void acceptPwd(){
		...
		MD5Encrypter md5 = new MD5Encrypter(passwd);
		boolean found = false;
		for (User u :  users){
			if (u.getUserName().equals(userName)&&u.getPasswd().equals(md5.getMd5())){
	...
	public void actionPerformed(ActionEvent event) {
		...
		if (command.equals(MessageBundle.getMessage("angal.menu.cancel"))){
			logger.warn("Login cancelled.");
			dispose();
		}
		else if (command.equals(MessageBundle.getMessage("angal.menu.submit"))){
			acceptPwd();			
		}
	}
	...
	private class LoginPanel extends JPanel {
		...
		public LoginPanel(Login myFrame) {
			...
			*/
			...
			JButton submit = new JButton(MessageBundle.getMessage("angal.menu.submit"));
			submit.setMnemonic(KeyEvent.VK_S);
			JButton cancel = new JButton(MessageBundle.getMessage("angal.menu.cancel"));
