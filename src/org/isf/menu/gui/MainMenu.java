...
public class MainMenu extends JFrame implements ActionListener,
	...
	// active or not active XMPP
	private boolean flag_Xmpp = false;
	// debug mode
	private boolean debug = false;
	// pregnancy flag
	private boolean pregnancycare = false;
	
	public void loginInserted(AWTEvent e) {
		if (e.getSource() instanceof User) {
			myUser = (User) e.getSource();
			int sessionID = manager.newSession(myUser);
			mySession = manager.getSessionByID(sessionID);
			MDC.put("OHUser", myUser.getUserName());
			MDC.put("OHUserGroup", myUser.getUserGroupName());
			logger.info("Logging: \"" + myUser.getUserName() + "\" user has logged the system (SID: " + sessionID + ").");
		}
	}
	...
	public MainMenu() {	
		...
		try{
			...
			if (debug) {
				logger.debug("Debug: OpenHospital in debug mode.");
			}
			pregnancycare = GeneralData.PREGNANCYCARE;
			...
			pregnancycare = false; // default for property not found
		}
		...
		// if not pregnancycare mode remove "pregnancy" and "pregnancyexam" menu
		if (!pregnancycare) {
			ArrayList<UserMenuItem> junkMenu = new ArrayList<UserMenuItem>();
			Iterator<UserMenuItem> it = myMenu.iterator();
			while (it.hasNext()) {
				UserMenuItem umi = it.next();
				if (umi.getCode().equalsIgnoreCase("PREGNANCY")
						|| umi.getMySubmenu().equalsIgnoreCase("PREGNANCY"))
					junkMenu.add(umi);
				if (umi.getCode().equalsIgnoreCase("PREGNANCYEXAM")
						|| umi.getMySubmenu().equalsIgnoreCase("PREGNANCYEXAM"))
					junkMenu.add(umi);
			}
			Iterator<UserMenuItem> altIt = junkMenu.iterator();
			while (altIt.hasNext()) {
	...
	private void actionExit(int status) {
		...
		else manager.closeSession(mySession.getSessionID());
		logger.info("\n\n=====================\n OpenHospital closed \n=====================\n");
		System.exit(status);
	}	
