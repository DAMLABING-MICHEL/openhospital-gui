...
public class IoOperations {
	...
	public boolean newUser(User user) throws OHException {
		...
		String sqlString = "INSERT INTO USER (US_ID_A , US_UG_ID_A, US_PASSWD, US_DESC ) VALUES (?, ?, MD5(?), ?)";
	...
	public boolean updatePassword(User user) throws OHException {
		...
		String sqlstring = "UPDATE USER SET US_PASSWD = MD5(?) WHERE US_ID_A = ?";
		parameters.add(user.getPasswd());
		parameters.add(user.getUserName());
	 ...
	 * @throws OHException
	 */
	public int newSession(User user) throws OHException {
		DbQueryLogger dbQuery = new DbQueryLogger();
		int sessionID;
		List<Object> parameters = Collections.<Object>singletonList(user.getUserName());
		String sqlString = "INSERT INTO SESSION (SES_US_ID_A) VALUES (?)";
		try {	
			ResultSet result = dbQuery.setDataReturnGeneratedKeyWithParams(sqlString, parameters, true);
	...
	/**
	 * gets the {@link User}'s session
	 * 
	 * @param sessionID - the session ID
	 * @return user's {@link Session}, <code>null</code> if no Session is found
	 * @throws OHException
	 */
	public Session getSessionByID(int sessionID) throws OHException {
		Session session = null;
		DbQueryLogger dbQuery = new DbQueryLogger();
		List<Object> parameters = Collections.<Object>singletonList(sessionID);
		String sqlString = "SELECT * FROM SESSION WHERE SES_ID = ?";
		
		ResultSet resultSet = dbQuery.getDataWithParams(sqlString, parameters, true);
		try {	
			
			if (resultSet.next()) session = toSession(resultSet);
	...
	/**
	 * gets the {@link User}'s sessions
	 * 
	 * @param userName - the {@link User}'s name
	 * @param limit - the number of sessions to retrieve
	 * @return user's list of {@link Session}s, <code>null</code> if no Session is found
	 * @throws OHException
	 */
	public ArrayList<Session> getSessionByUser(String userName, int limit) throws OHException {
		DbQueryLogger dbQuery = new DbQueryLogger();
		ArrayList<Session> sessionList = new ArrayList<Session>();
		List<Object> parameters = new ArrayList<Object>();
		
		StringBuilder sqlString = new StringBuilder("SELECT * FROM SESSION WHERE SES_US_ID_A LIKE ? ");
		parameters.add(userName);
		
		sqlString.append("ORDER BY SES_ID DESC ");
	 ...
	 * @throws OHException
	 */
	public boolean closeSession(int sessionID) throws OHException {
		DbQueryLogger dbQuery = new DbQueryLogger();
		List<Object> parameters = new ArrayList<Object>();
		String sqlString = "UPDATE SESSION SET SES_LOGOUT = NOW() WHERE SES_ID = ?";
		//parameters.add(new Date());
		parameters.add(sessionID);
		
		boolean result = dbQuery.setDataWithParams(sqlString, parameters, true);
	...
	/**
	 * Extract {@Link Session} object from resulset.
	 * @param resultSet
	 * @return {@Link Session}
	 * @throws SQLException
	 */
	public Session toSession(ResultSet resultSet) throws SQLException {
		Session session = new Session(resultSet.getInt("SES_ID"),
				resultSet.getString("SES_US_ID_A"),
				convertToGregorianCalendar(resultSet.getTimestamp("SES_LOGIN")),
				convertToGregorianCalendar(resultSet.getTimestamp("SES_LOGOUT")));
		return session;
	}
