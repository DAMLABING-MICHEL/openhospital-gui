...
import org.isf.utils.time.RememberOpdWard;
...
public class OpdEditExtended extends JDialog implements PatientInsertExtended.PatientListener, PatientInsert.PatientListener {
	...
	private void setPatient(Patient p) {
			jFieldAge.setText(String.valueOf(p.getFormattedAge()));
			...
			jOpdNumField.setText(p.getPreviousCode());
			getDiseaseBox();
			getDiseaseBox2();
			getDiseaseBox3();
	}
	...
	private JPanel getDataPanel() {
		if (jPanelData == null) {
			...
			gbl_jPanelData.columnWidths = new int[] {80,40,20,80, 0,20};
			gbl_jPanelData.rowHeights = new int[]{20,20,20,20,20,20,20,20};
			gbl_jPanelData.columnWeights = new double[]{0.0, 0.1, 0.0, 1.0, 1.0, 0.0};
			...
			GridBagConstraints gbc_jComboBoxWard = new GridBagConstraints();
			gbc_jComboBoxWard.insets = new Insets(0, 0, 5, 5);
			gbc_jComboBoxWard.fill = GridBagConstraints.HORIZONTAL;
			gbc_jComboBoxWard.gridx = 4;
			gbc_jComboBoxWard.gridy = 0;
			jPanelData.add(getJComboBoxWard(), gbc_jComboBoxWard);
			...
			gbc_jLabelDiseaseTypeBox.gridwidth = 5;
			...
			gbc_jFieldLastOpdVisit.gridwidth = 5;
			...
			gbc_jFieldLastOpdNote.gridwidth = 5;
			...
			try {
				...
				opdDateFieldCal.addPropertyChangeListener("date", new PropertyChangeListener() {
					...
					public void propertyChange(PropertyChangeEvent evt) {
						...
						if (opdPatient != null && newValue.before(opdPatient.getBirthDate())) {
							JOptionPane.showMessageDialog(OpdEditExtended.this, "The patient was not yet born at the selected date!!!");
							opdDateFieldCal.setDate((Date) evt.getOldValue());
							return;
						}
						opdDateFieldCal.setDate(newValue);
						dateOpd.setTime(newValue);
						getDiseaseBox();
						getDiseaseBox2();
						getDiseaseBox3();
					}
	...
	private JComboBox getDiseaseBox() {
		if (diseaseBox1 == null) {
			...
			diseaseBox1.addItemListener(new ItemListener() {
		...
		Disease found = null;
		String opdDisease1 = opd.getDisease();
		diseaseBox1.removeAllItems();
		diseaseBox1.addItem("");
		for (Disease elem : diseasesOPD) {
			
			boolean ok = true;
			if (opdPatient != null && !elem.isMale() && opdPatient.getSex() == 'M') ok = false; //skip female diseases for male patients
			if (opdPatient != null && !elem.isFemale() && opdPatient.getSex() == 'F') ok = false; //skip male diseases for female patients
			if (opdPatient != null && elem.getMinimumMonths() != 0 && opdPatient.getMonthsAtDate(dateOpd) < elem.getMinimumMonths()) ok = false; //skip diseases for young patients
			if (opdPatient != null && elem.getMaximumMonths() != 0 && opdPatient.getMonthsAtDate(dateOpd) > elem.getMaximumMonths()) ok = false; //skip diseases for old patients
			if (ok) {
				if (((DiseaseType)diseaseTypeBox.getSelectedItem()).equals(allType))
					...
					diseaseBox1.addItem(elem);
				if(!insert && opdDisease1!=null){
					if(opdDisease1.equals(elem.getCode())){
						found = elem;}
	...
	private JComboBox getDiseaseBox2() {
		...
		Disease found=null;
		String opdDisease2 = opd.getDisease2();
		diseaseBox2.removeAllItems();
		diseaseBox2.addItem("");
		...
		for (Disease elem : diseasesOPD) {
			...
			if (opdPatient != null && elem.getMinimumMonths() != 0 && opdPatient.getMonthsAtDate(dateOpd) < elem.getMinimumMonths()) ok = false; //skip diseases for young patients
			if (opdPatient != null && elem.getMaximumMonths() != 0 && opdPatient.getMonthsAtDate(dateOpd) > elem.getMaximumMonths()) ok = false; //skip diseases for old patients
			if (ok) diseaseBox2.addItem(elem);
			if(!insert && opdDisease2!=null){
				if(opdDisease2.equals(elem.getCode())){
					found = elem;}
			} 
		}
		if (found!= null) {
			diseaseBox2.setSelectedItem(found);
		} else { //try in the canceled diseases
			if (opdDisease2!=null) {
				for (Disease elem : diseasesAll) {
					
					if (opdDisease2.compareTo(elem.getCode()) == 0) {
	...
	private JComboBox getDiseaseBox3() {
		...
		Disease found=null;
		String opdDisease3 = opd.getDisease3();
		diseaseBox3.removeAllItems();
		diseaseBox3.addItem("");
		...
		for (Disease elem : diseasesOPD) {
			...
			if (opdPatient != null && elem.getMinimumMonths() != 0 && opdPatient.getMonthsAtDate(dateOpd) < elem.getMinimumMonths()) ok = false; //skip diseases for young patients
			if (opdPatient != null && elem.getMaximumMonths() != 0 && opdPatient.getMonthsAtDate(dateOpd) > elem.getMaximumMonths()) ok = false; //skip diseases for old patients
			if (ok) diseaseBox3.addItem(elem);
			if(!insert && opdDisease3!=null){
				if(opdDisease3.equals(elem.getCode())){
					found = elem;}
			}
		}
		if (found!= null) {
			diseaseBox3.setSelectedItem(found);
		} else { //try in the canceled diseases
			if (opdDisease3!=null) {	
				for (Disease elem : diseasesAll) {
					if (opdDisease3.compareTo(elem.getCode()) == 0) {
	...
	private VoLimitedTextField getJTextPatientSrc() {
		if (jTextPatientSrc == null) {
			jTextPatientSrc = new VoLimitedTextField(12,12);
	...
	private JComboBox getSearchBox() {
		if (jComboPatResult == null) {
			...
			jComboPatResult.setPreferredSize(new Dimension(250, 24));
			if (opdPatient != null) {
		...
		if (okButton == null) {
			...
			okButton.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					char sex = opdPatient.getSex();
					int months = opdPatient.getMonthsAtDate(dateOpd);
					String newPatient="";
					...
					Ward ward = null;
					try {
						ward = (Ward) jComboBoxWard.getSelectedItem();
					} catch (ClassCastException e1) {
						JOptionPane.showMessageDialog(OpdEditExtended.this, "Please select a Ward");
						return;
					}
					...
					if (diseaseBox1.getSelectedIndex()>0) {
						...
						diseaseDesc=disease.getDescription();
						diseaseType=((Disease)diseaseBox1.getSelectedItem()).getType();
						diseaseTypeDesc=diseaseType.getDescription();
						if (!disease.isFemale() && sex == 'F') {
							JOptionPane.showMessageDialog(OpdEditExtended.this, "Selected disease 1 is not releted to female patients");
							return;
						}
						if (!disease.isMale() && sex == 'M') {
							JOptionPane.showMessageDialog(OpdEditExtended.this, "Selected disease 1 is not releted to male patients");
							return;
						}
						if (disease.getMinimumMonths() != 0 && months < disease.getMinimumMonths()) {
							JOptionPane.showMessageDialog(OpdEditExtended.this, "the patient is too young for selected disease 1 (at the visit date)");
							return;
						}
						if (disease.getMaximumMonths() != 0 && months > disease.getMaximumMonths()) {
							JOptionPane.showMessageDialog(OpdEditExtended.this, "the patient is too old for selected disease 1 (at the visit date)");
							return;
						}
					...
					if (diseaseBox2.getSelectedIndex()>0) {
						...
						if (!disease2.isMale() && sex == 'M') {
							JOptionPane.showMessageDialog(OpdEditExtended.this, "Selected disease 2 is not releted to male patients");
							return;
						} 
						if (disease2.getMinimumMonths() != 0 && months < disease2.getMinimumMonths()) {
							JOptionPane.showMessageDialog(OpdEditExtended.this, "the patient is too young for selected disease 2 (at the visit date)");
							return;
						}
						if (disease2.getMaximumMonths() != 0 && months > disease2.getMaximumMonths()) {
							JOptionPane.showMessageDialog(OpdEditExtended.this, "the patient is too old for selected disease 2 (at the visit date)");
							return;
						}
					...
					if (diseaseBox3.getSelectedIndex()>0) {
						...
						if (!disease3.isMale() && sex == 'M') {
							JOptionPane.showMessageDialog(OpdEditExtended.this, "Selected disease 3 is not releted to male patients");
							return;
						} 
						if (disease3.getMinimumMonths() != 0 && months < disease3.getMinimumMonths()) {
							JOptionPane.showMessageDialog(OpdEditExtended.this, "the patient is too young for selected disease 3 (at the visit date)");
							return;
						}
						if (disease3.getMaximumMonths() != 0 && months > disease3.getMaximumMonths()) {
							JOptionPane.showMessageDialog(OpdEditExtended.this, "the patient is too old for selected disease 3 (at the visit date)");
							return;
						}
...
//					}
					...
					if (insert){
						...
						RememberOpdWard.setLastOpdWard(ward);
						result = opdManager.newOpd(opd) > 0;
						if (result) {
							fireSurgeryInserted();
						}
	...
	private JComboBox getJComboBoxWard() {
		...
		if (jComboBoxWard == null) {
			...
			wardList = wardManager.getWardsOPD();
			Ward ward2 = null;
			...
			for (Ward ward : wardList) {
				...
				if (insert) {
					if (RememberOpdWard.getLastOpdWard()!=null) {
						ward2 = RememberOpdWard.getLastOpdWard();
					}	
					
					
				}else if (!insert && opd.getWard() != null) {
					if (opd.getWard().equals(ward.getCode())) {
						ward2 = ward;
					}
				}
			}
			if (ward2 != null) {
				jComboBoxWard.setSelectedItem(ward2);
			} else {
