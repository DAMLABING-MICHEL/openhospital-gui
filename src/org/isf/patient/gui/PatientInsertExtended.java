...
import org.isf.location.manager.LocationManager;
...
public class PatientInsertExtended extends JDialog {
	...
	// Second Name Components:
	private JPanel jPreviousCode = null;
	private JPanel jPreviousCodeLabelPanel = null;
	private JPanel jPreviousCodeFieldPanel = null;
	private JLabel jPreviousCodeLabel = null;
	private JTextField jPreviousCodeTextField = null;
	...
	private JComboBox jAddressComboBox = new JComboBox();
	...
	private JComboBox jCityComboBox = null;
	...
	// Second Name Components:
	private JPanel jInsuranceName = null;
	private JTextField jInsuranceNameTextField = null;
	...
	private ArrayList<Location> locations = locMan.getLocation();
	private ArrayList<String> cities = locMan.getCity();
		...
		if (jHeightAndWeightButton == null) {
			...
			jHeightAndWeightButton.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					...
					if (exists) {
						//TODO: do something to manage patient WEIGHT and HEIGHT independently from malnutrition
//						MalnutritionVisit malnutrition = new MalnutritionVisit(patient);
//						malnutrition.setType(MalnutritionVisit.HOSPITALIZATION);
//						List<MalnutritionVisit> malnutritionVisitList = new ArrayList<MalnutritionVisit>();
//						malnutritionVisitList.add(malnutrition);
//						new MalnutritionVisitEdit(malnutritionVisitList);
					}
		...
		if (jOkButton == null) {
			...
			jOkButton.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					String previousCode = jPreviousCodeTextField.getText().trim();
					...
					if (previousCode.equals("")) {
						JOptionPane.showMessageDialog(PatientInsertExtended.this, "Please insert the OPD number (old code)");
						return;
					}
					if (insert) {
						...
						if (ok) {
							...
							patient.setPreviousCode(previousCode);
							...
							String patAddress = (String) jAddressComboBox.getSelectedItem();
								patient.setAddress(patAddress == null ? "" : patAddress);
							String patCity = (String) jCityComboBox.getSelectedItem();
								patient.setCity(patCity == null ? "" : patCity);
							...
							patient.setInsuranceName(jInsuranceNameTextField.getText().trim());
...
//							}
						...
						patient.setTaxCode(jTaxCodeTextField.getText().trim());
						patient.setAddress((String) jAddressComboBox.getSelectedItem());
						patient.setCity((String) jCityComboBox.getSelectedItem());
						patient.setNextKin(jNextKinTextField.getText().trim());
						patient.setTelephone(jTelephoneTextField.getText().replaceAll(" ", ""));
						patient.setMother_name(jMotherNameTextField.getText().trim());
...
//						}
	...
	private JPanel getJPreviousCodePanel() {
		if (jPreviousCodeLabelPanel == null) {
			jPreviousCodeLabelPanel = new JPanel();
			jPreviousCodeLabelPanel.add(getJPreviousCodeLabel(), BorderLayout.EAST);
		}
		return jPreviousCodeLabelPanel;
	}
	...
	private JTextField getJPreviousCodeTextField() {
		if (jPreviousCodeTextField == null) {
			jPreviousCodeTextField = new JTextField(15);
			if (!insert)
				jPreviousCodeTextField.setText(patient.getPreviousCode());

		}
		return jPreviousCodeTextField;
	}
	...
	private JComboBox getJAddressComboBox() {
		if (jAddressComboBox == null) {
			...
			jAddressComboBox.addItem("");
		}
		jAddressComboBox.setPreferredSize(new Dimension(168,24));
	...
	private JComboBox getJCityComboBox() {
		if (jCityComboBox == null) {
			...
			jCityComboBox.addItem("");
			jCityComboBox.setPreferredSize(new Dimension(168,24));
			for (String city : cities) {
				jCityComboBox.addItem(city);
			}
			jCityComboBox.addActionListener(new ActionListener() {
				
				@Override
				public void actionPerformed(ActionEvent e) {
					jAddressComboBox.removeAllItems();
					jAddressComboBox.addItem("");
					String theCity = (String) jCityComboBox.getSelectedItem();
					for (Location loc : locations) {
						if (loc.getCity().equals(theCity))
							jAddressComboBox.addItem(loc.getAddress());
					}
					String patAddress = patient.getAddress();
					if (patAddress != null)
						jAddressComboBox.setSelectedItem(patAddress);
				}
			});
			if (!insert) {
				String patCity = patient.getCity();
				if (patient.getCity() != null)
					jCityComboBox.setSelectedItem(patCity);
			}
		}
	 ...
	 * @return javax.swing.JTextField
	 */
	private JTextField getJInsuranceNameTextField() {
		if (jInsuranceNameTextField == null) {
			jInsuranceNameTextField = new JTextField(15);
			if (!insert)
				jInsuranceNameTextField.setText(patient.getInsuranceName());
		}
		return jInsuranceNameTextField;
	}
	...
	private JLabel getJPreviousCodeLabel() {
		if (jPreviousCodeLabel == null) {
			jPreviousCodeLabel = new JLabel();
			jPreviousCodeLabel.setText("OPD No. *");
		}
		return jPreviousCodeLabel;
	}
	...
	private JPanel getJAnagraphPanel() {
		if (jAnagraphPanel == null) {
			...
			jAnagraphPanel.add(getJPreviousCode(), null);
	...
	private JPanel getJPreviousCodePanel1() {
		if (jPreviousCodeFieldPanel == null) {
			jPreviousCodeFieldPanel = new JPanel();
			jPreviousCodeFieldPanel.add(getJPreviousCodeTextField(), null);
		}
		return jPreviousCodeFieldPanel;
	}
	...
	private JPanel getJPreviousCode() {
		if (jPreviousCode == null) {
			jPreviousCode = new JPanel();
			jPreviousCode.setLayout(new BorderLayout());
			jPreviousCode.add(getJPreviousCodePanel(), java.awt.BorderLayout.WEST);
			jPreviousCode.add(getJPreviousCodePanel1(), java.awt.BorderLayout.EAST);
		}
		return jPreviousCode;
	}
	...
	private JPanel getJAgeType() {
		if (jAgeType == null) {
			jAgeType = new JPanel();
			jAgeType = setMyBorder(jAgeType, "Age *");
...
//			jAgeTypeSelection = getJAgeDescription();
	...
	private JPanel getJCity() {
		if (jCity == null) {
			jCity = new JPanel();
			GridBagLayout gbl_jCity = new GridBagLayout();
			gbl_jCity.columnWidths = new int[]{10, 1, 0};
			gbl_jCity.rowHeights = new int[]{1, 0};
			gbl_jCity.columnWeights = new double[]{0.3, 0.7, Double.MIN_VALUE};
			gbl_jCity.rowWeights = new double[]{0.0, Double.MIN_VALUE};
			jCity.setLayout(gbl_jCity);
			
			jCityLabel = new JLabel();
			jCityLabel.setText(MessageBundle.getMessage("angal.patient.city"));
			GridBagConstraints gbc_jCityLabelPanel = new GridBagConstraints();
			gbc_jCityLabelPanel.anchor = GridBagConstraints.WEST;
			gbc_jCityLabelPanel.insets = new Insets(0, 5, 0, 0);
			gbc_jCityLabelPanel.gridx = 0;
			gbc_jCityLabelPanel.gridy = 0;
			jCity.add(jCityLabel, gbc_jCityLabelPanel);
			
			GridBagConstraints gbc_jCityFieldPanel = new GridBagConstraints();
			gbc_jCityFieldPanel.anchor = GridBagConstraints.EAST;
			gbc_jCityFieldPanel.insets = new Insets(0, 12, 0, 5);
			gbc_jCityFieldPanel.gridx = 1;
			gbc_jCityFieldPanel.gridy = 0;
			jCity.add(getJCityComboBox(), gbc_jCityFieldPanel);
		}
		return jCity;
	}
	...
	private JPanel getJAddressPanel() {
		if (jAddress == null) {
			jAddress = new JPanel();
			GridBagLayout gbl_jAddress = new GridBagLayout();
			gbl_jAddress.columnWidths = new int[]{10, 1, 0};
			gbl_jAddress.rowHeights = new int[]{1, 0};
			gbl_jAddress.columnWeights = new double[]{0.3, 0.7, Double.MIN_VALUE};
			gbl_jAddress.rowWeights = new double[]{0.0, Double.MIN_VALUE};
			jAddress.setLayout(gbl_jAddress);
			
			jAddressLabel = new JLabel();
			jAddressLabel.setText(MessageBundle.getMessage("angal.patient.address"));
			GridBagConstraints gbc_jAddressLabelPanel = new GridBagConstraints();
			gbc_jAddressLabelPanel.anchor = GridBagConstraints.WEST;
			gbc_jAddressLabelPanel.insets = new Insets(0, 5, 0, 0);
			gbc_jAddressLabelPanel.gridx = 0;
			gbc_jAddressLabelPanel.gridy = 0;
			jAddress.add(jAddressLabel, gbc_jAddressLabelPanel);
			
			GridBagConstraints gbc_jAddressFieldPanel = new GridBagConstraints();
			gbc_jAddressFieldPanel.anchor = GridBagConstraints.EAST;
			gbc_jAddressFieldPanel.insets = new Insets(0, 0, 0, 5);
			gbc_jAddressFieldPanel.gridx = 1;
			gbc_jAddressFieldPanel.gridy = 0;
			jAddress.add(getJAddressComboBox(), gbc_jAddressFieldPanel);
	...
	private JPanel getJInsurancePanel() {
		if (jInsurancePanel == null) {
			...
			jInsuranceName = new JPanel();
			jInsuranceName.add(getJInsuranceNameTextField());
			jInsurancePanel.add(jInsuranceName, BorderLayout.SOUTH);
		}
	...
	private JPanel getJRightPanel() {
		if (jRightPanel == null) {
			...
			try {
				...
				int code = patient.getCode();
				Image image = patient.getPhoto();
				photoPanel = new PatientPhotoPanel(this, code, image);
