...
public class PatientBrowserManager {
	...
	/**
	 * Returns the full list of Patients not logically deleted
	...
	public ArrayList<Patient> getPatients(String hint) {
		return getPatients(hint, false);
	}

	/**
	 * Return an ArrayList of {@link Patient} objects.
	 * @param hint String, if not <code>null</code> it will be use to search for patient with either id,
	 *                               firs name or last name starting with {@param hint} value.
	 * @param searchByPreviousCode boolean if set to true, {@param hint} is used to search also by previousCode (this
	 *                             field stores in fact the opd number).
	 * @return {@link ArrayList} of {@link Patient} objects.
     */
	public ArrayList<Patient> getPatients(String hint, boolean searchByPreviousCode) {
		try {
			return ioOperations.getPatients(hint, searchByPreviousCode);
		} catch (OHException e) {
			JOptionPane.showMessageDialog(null, e.getMessage());
	 ...
	 * 
	 * @param patient {@link Patient} object.
