...
public class IoOperations {
	...
	public ArrayList<Patient> getPatients() throws OHException {
		...
		try {
			...
			while (resultSet.next()) {
				pPatient.add(parsePatientRecord(resultSet, false));
			}
	...
	public ArrayList<Patient> getPatientsWithHeightAndWeight(String regex) throws OHException {
		...
		StringBuilder queryBld = new StringBuilder(
				"SELECT * FROM PATIENT LEFT JOIN (SELECT MLN_PAT_ID, MLN_HEIGHT AS PAT_HEIGHT, MLN_WEIGHT AS PAT_WEIGHT FROM MALNUTRITIONCONTROL GROUP BY MLN_PAT_ID ORDER BY MLN_DATE_SUPP DESC) AS HW ON PAT_ID = HW.MLN_PAT_ID WHERE (PAT_DELETED='N' or PAT_DELETED is null) ");
		...
		if (regex != null && !regex.equals("")) {
			...
			for (int i = 0; i < s1.length; i++) {
				queryBld.append("AND CONCAT(PAT_ID, LOWER(PAT_SNAME), LOWER(PAT_FNAME), LOWER(PAT_NOTE), LOWER(PAT_TAXCODE), LOWER(PAT_TELE), LOWER(PAT_PCODE)) ");
		...
		try {
			...
			pPatient = new ArrayList<Patient>(resultSet.getFetchSize());
			while (resultSet.next()) {
				pPatient.add(parsePatientRecord(resultSet, true));
			}
		} catch (SQLException e) {
			throw new OHException(MessageBundle.getMessage("angal.sql.problemsoccurredwiththesqlistruction"), e);
		} catch (IOException e) {
	...
	/**
	 * Returns a list of {@link Patient} object whose name or id starts with the given hint.
	 * @param hint String representing a hint for the beginning of either the name or the code of the {@link Patient}.
	 * @param searchByPreviousCode boolean, if true, the hint is used to search by previousCode (PAT_PCODE field) as well.
	 * @return ArrayList of {@link Patient} objects.
	 * @throws OHException
     */
	public ArrayList<Patient> getPatients(String hint, boolean searchByPreviousCode) throws OHException {
		if (hint == null) {
			return getPatients();
		}
		hint += "%";
		...
		String query = "SELECT * FROM PATIENT" +
				" LEFT JOIN" +
				" (SELECT MLN_PAT_ID, MLN_HEIGHT AS PAT_HEIGHT, MLN_WEIGHT AS PAT_WEIGHT FROM MALNUTRITIONCONTROL" +
				" GROUP BY MLN_PAT_ID" +
				" ORDER BY MLN_DATE_SUPP DESC) AS HW" +
				" ON PAT_ID = HW.MLN_PAT_ID" +
				" WHERE" +
				" (PAT_DELETED='N' OR PAT_DELETED IS NULL)" +
				" AND (PAT_ID LIKE ? OR LOWER(PAT_NAME) LIKE ?";
		if (searchByPreviousCode)
			query += " OR PAT_PCODE LIKE ?";
		query += ")" +
		...
		try {
			...
			while (resultSet.next()) {
				patients.add(parsePatientRecord(resultSet, true));
			}
		} catch (SQLException e) {
			throw new OHException(MessageBundle.getMessage("angal.sql.problemsoccurredwiththesqlistruction"), e);
		} catch (IOException e) {
	...
	public Patient getPatient(String name) throws OHException {
		...
		try {
			...
			patient = new Patient();
			while (resultSet.next()) {
				patient = parsePatientRecord(resultSet, false);
			}
		} catch (SQLException e) {
			throw new OHException(MessageBundle.getMessage("angal.sql.problemsoccurredwiththesqlistruction"), e);
		} catch (IOException e) {
	...
	public boolean newPatient(Patient patient) throws OHException {
		...
		try {
			String query = "INSERT INTO PATIENT (PAT_NAME, PAT_FNAME, PAT_SNAME, PAT_BDATE, PAT_AGE, PAT_AGETYPE, PAT_SEX, PAT_ADDR, PAT_CITY, PAT_NEXT_KIN, PAT_TELE, PAT_MOTH_NAME, PAT_MOTH, PAT_FATH_NAME, PAT_FATH, PAT_BTYPE, PAT_ESTA, PAT_PTOGE, PAT_NOTE, PAT_TAXCODE, PAT_PCODE, PAT_ESTA_NAME, PAT_PHOTO) " +
					"VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
			...
			params.add(patient.getPreviousCode());
			params.add(patient.getInsuranceName());
	...
	public boolean updatePatient(Patient patient, boolean check) throws OHException {
		...
		try {
			...
			String query = "UPDATE PATIENT SET PAT_FNAME = ?, PAT_SNAME = ?, PAT_NAME  = ?, PAT_BDATE = ?, PAT_AGE = ?, PAT_AGETYPE = ?, PAT_SEX = ?, PAT_ADDR = ?, PAT_CITY = ?, PAT_NEXT_KIN = ?, PAT_TELE = ?, PAT_MOTH = ?, PAT_MOTH_NAME = ?, PAT_FATH = ?, PAT_FATH_NAME = ?, PAT_BTYPE = ?, PAT_ESTA = ?, PAT_PTOGE = ?, PAT_NOTE = ?, PAT_TAXCODE = ?, PAT_PCODE = ?, PAT_ESTA_NAME = ?, PAT_LOCK = ?, PAT_PHOTO = ? WHERE PAT_ID = ?";
			...
			/*
			 * Occorre aggiornare il model perche' il paziente non viene riletto dal DB.
			 */
			if (result)
	...
	/**
	 * Utility to parse a {@link Patient} object from the database.
	 * @param resultSet {@link ResultSet} object returned from the db.
	 * @param extended boolean, set to <code>true</code> to include patient's weight and height.
	 * @return {@link ArrayList} of {@link Patient} objects.
	 * @throws SQLException
	 * @throws IOException
     */
	public Patient parsePatientRecord(ResultSet resultSet, boolean extended) throws SQLException, IOException {
		...
		patient.setPreviousCode(resultSet.getString("PAT_PCODE"));
		patient.setInsuranceName(resultSet.getString("PAT_ESTA_NAME"));
		// Image
