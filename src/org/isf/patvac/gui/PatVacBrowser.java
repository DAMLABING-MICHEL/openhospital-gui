...
public class PatVacBrowser extends ModalJFrame implements ActionListener{
	...
	private String[] pColums = { MessageBundle.getMessage("angal.patvac.datem"), MessageBundle.getMessage("angal.patvac.patientm"), MessageBundle.getMessage("angal.patvac.sexm"),MessageBundle.getMessage("angal.patvac.agem"), MessageBundle.getMessage("angal.patvac.vaccinem"), MessageBundle.getMessage("angal.patvac.vaccinetypem")};
	...
	private JButton getButtonNew(){
	   if (buttonNew == null) {
		   buttonNew = new JButton(MessageBundle.getMessage("angal.patvac.new"));
	...
	private JButton getButtonEdit(){
		...
		if (buttonEdit == null) {
			buttonEdit = new JButton(MessageBundle.getMessage("angal.patvac.edit"));
			buttonEdit.setMnemonic(KeyEvent.VK_S);
			buttonEdit.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent event) {
					if (jTable.getSelectedRow() < 0) {
						JOptionPane.showMessageDialog(null,
								MessageBundle.getMessage("angal.patvac.pleaseselectarow"), MessageBundle.getMessage("angal.hospital"),
	...
	private JButton getButtonDelete(){
		if (buttonDelete == null) {
			buttonDelete = new JButton(MessageBundle.getMessage("angal.patvac.delete"));
	...
	private JButton getCloseButton() {
		if (buttonClose == null) {
			buttonClose = new JButton(MessageBundle.getMessage("angal.patvac.close"));
	...
	private JPanel getPatientSearchPanel() {
		if (jPatientSearchPanel == null) {
			...
			// patientComboBox.setBounds(labelWidth + 180, DeltaBetweenLabels -
			// 8,
			// 270, 20);
...
//		jTextPatientSrc.setText(selectedPatient.getName());
	...
	private JPanel getDatePanel (){
		...
		JPanel label1Panel = new JPanel(new FlowLayout(FlowLayout.CENTER));
		label1Panel.add(new JLabel(MessageBundle.getMessage("angal.patvac.date") +": "+ MessageBundle.getMessage("angal.patvac.from")), null);
		...
		label1Panel = new JPanel(new FlowLayout(FlowLayout.CENTER));
		label1Panel.add(new JLabel(MessageBundle.getMessage("angal.patvac.date") +": "+MessageBundle.getMessage("angal.patvac.to") +"     "), null);
		datePanel.add(label1Panel);
