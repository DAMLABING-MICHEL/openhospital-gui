...
public class ListBrowser extends ModalJFrame  implements ListListener{
	...
	private JButton getJButtonClose() {
		if (jButtonClose == null) {
			...
			jButtonClose.setText(MessageBundle.getMessage("angal.priceslist.close")); //$NON-NLS-1$
	...
	private JButton getJButtonDelete() {
		if (jButtonDelete == null) {
			...
			jButtonDelete.setText(MessageBundle.getMessage("angal.priceslist.delete")); //$NON-NLS-1$
					...
					if (jTablePriceLists.getSelectedRow() < 0) {
						...
						if (newName != null) {
							...
							if (r == JOptionPane.OK_OPTION) {
								try {
									multiplier = (Double) jSpinnerQty.getValue();
									if (multiplier == 0.) {
							...
							if (multiplier != 1.) {
								...
								r = JOptionPane.showConfirmDialog(ListBrowser.this, 
									new Object[] { MessageBundle.getMessage("angal.priceslist.rounduptothenearest" + " (zero for no rounding)"), jSpinnerQty },
...
//										}
							...
							boolean result = false;
							result = listManager.copyList(copiedList, multiplier, step);
	...
	private JButton getJButtonEdit() {
		if (jButtonEdit == null) {
			...
			jButtonEdit.setText(MessageBundle.getMessage("angal.priceslist.edit")); //$NON-NLS-1$
	...
	private JButton getJButtonNew() {
		if (jButtonNew == null) {
			...
			jButtonNew.setText(MessageBundle.getMessage("angal.priceslist.new")); //$NON-NLS-1$
