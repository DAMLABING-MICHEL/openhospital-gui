...
public class PricesBrowser extends ModalJFrame {
    ...
    private ArrayList<Medical> mediArray = mediManager.getMedicalsSortedByCode();
	...
	private JButton getPrintTableButton() {
		if (jPrintTableButton == null) {
			jPrintTableButton = new JButton("Print");
	...
	private JButton getJButtonSave() {
		if (jButtonSave == null) {
			...
			jButtonSave.setText(MessageBundle.getMessage("angal.priceslist.savem")); //$NON-NLS-1$
			...
			jButtonSave.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent event) {
					...
					try {
						 ...
						 */
						BusyState.setBusyState(PricesBrowser.this, true);
						...
						BusyState.setBusyState(PricesBrowser.this, false);
					}
				}
	...
	private void updateFromDB() {
	    ...
	    mediArray = mediManager.getMedicalsSortedByCode();
	...
	private PriceNode getTreeContent() {
	    ...
	    for(Medical med: mediArray){
	    	...
	    	double priceValue = p != null ? p.getPrice() : 0.;
		    medNodes.addItem(new PriceNode(new Price(0, cCategories[2], med.getCode().toString(), med.getDescriptionWithCode(), priceValue)));
	    }
