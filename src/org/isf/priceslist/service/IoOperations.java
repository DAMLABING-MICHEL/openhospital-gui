...
public class IoOperations {
	...
	public ArrayList<PriceList> getLists() throws OHException {
		ArrayList<PriceList> lists = null;
		String string = "SELECT * FROM PRICELISTS";
		DbQueryLogger dbQuery = new DbQueryLogger();
		try {
			ResultSet resultSet = dbQuery.getData(string, true);
			lists = new ArrayList<PriceList>(resultSet.getFetchSize());
			while (resultSet.next()) {
				lists.add(new PriceList(resultSet.getInt("LST_ID"),
	 ...
	 * @throws OHException 
	 */
	public boolean updatePrices(PriceList list, ArrayList<Price> prices) throws OHException {
		DbQueryLogger dbQuery = new DbQueryLogger();
	 ...
	 * @throws OHException 
	 */
	public boolean newList(PriceList list) throws OHException {
		DbQueryLogger dbQuery = new DbQueryLogger();
		ArrayList<Object> parameters = new ArrayList<Object>();
		boolean result = false;
		try {
	 ...
	 * @throws OHException 
	 */
	public boolean updateList(PriceList list) throws OHException {
		DbQueryLogger dbQuery = new DbQueryLogger();
	 ...
	 * @throws OHException 
	 */
	public boolean deleteList(PriceList list) throws OHException {
		DbQueryLogger dbQuery = new DbQueryLogger();
	 ...
	 * @throws OHException 
	 */
	public boolean copyList(PriceList list, double factor, double step) throws OHException {
		DbQueryLogger dbQuery = new DbQueryLogger();
		java.util.List<Object> parameters = new ArrayList<Object>();
		boolean result = false;
		int newID;
		try {
			...
			if(set.first()){
				...
				if (step > 0.) {
					...
					query = "INSERT INTO PRICES (PRC_LST_ID, PRC_GRP, PRC_ITEM, PRC_DESC, PRC_PRICE) "+
			        "SELECT ?, PRC_GRP, PRC_ITEM, PRC_DESC, PRC_PRICE * ? AS PRC_PRICE FROM PRICES WHERE PRC_LST_ID = ?";
					parameters.add(newID);
					parameters.add(factor);
					parameters.add(list.getId());
				}
