...
public class IoOperations {
	 ...
	 * @param missingMedical - the missing medical 
	 * @return <code>true</code> if the list has been saved, <code>false</code> otherwise.
	 * @throws OHException if an error occurs during the medical retrieving.
	 */
	...
	public boolean storePrescription(MedicalWardPrescription prescription, ArrayList<MedicalWardPrescriptionDetail> details) throws OHException  {
		String query = "INSERT INTO MEDICALDSRWARDPRESCRIPTION (MWP_DATE, MWP_PAT_ID, MWP_WRD_ID_A, MWP_BLL_ID, MWP_IPD, MWP_WRD_ID_B, MWP_DIS_ID_A, MWP_PRS_ID) VALUES (?,?,?,?,?,?,?,?)";
		...
		try {
			boolean result = true;
			int prescrID;
			ResultSet resultSet = dbQuery.setDataReturnGeneratedKeyWithParams(query.toString(), parameters, false);
			if (resultSet.next()) {
			...
			for (MedicalWardPrescriptionDetail detail : details) {
				...
				result = result && dbQuery.setDataWithParams(query.toString(), parameters, false);
			}
	...
	/**
	 * Sets a {@link MedicalWardPrescription} as 'fake-done' (status = 4);
	 * @param billID - the bill the prescription refers to
	 * @throws OHException
	 */
	public void fakeCloseMedicalPrescription(int billID) throws OHException {
		DbQueryLogger dbQuery = new DbQueryLogger();
		List<Object> parameters = Collections.<Object>singletonList(billID);
	...
	/**
	 * Sets a {@link MedicalWardPrescription} as 'fake-cancelled' (status = 5);
	 * @param billID - the bill the prescription refers to
	 * @throws OHException
	 */
	public void fakeDeleteMedicalPrescription(int billID) throws OHException {
		DbQueryLogger dbQuery = new DbQueryLogger();
		List<Object> parameters = Collections.<Object>singletonList(billID);
		
		StringBuilder query = new StringBuilder();
		query.append("UPDATE MEDICALDSRWARDPRESCRIPTION ");
		query.append("SET MWP_STATUS = 5 ");
		query.append("WHERE MWP_BLL_ID = ?");
