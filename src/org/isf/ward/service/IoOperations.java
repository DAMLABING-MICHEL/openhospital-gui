...
public class IoOperations {
	 ...
	 */
	public ArrayList<Ward> getWardNoMaternity() throws OHException {
		ArrayList<Ward> pWard = null;
		String string = "SELECT * FROM WARD WHERE WRD_IS_FEMALE = 0 ";
		DbQueryLogger dbQuery = new DbQueryLogger();
		try {
			ResultSet resultSet = dbQuery.getData(string, true);
			pWard = new ArrayList<Ward>(resultSet.getFetchSize());
			while (resultSet.next()) {
				pWard.add(new Ward(resultSet.getString("WRD_ID_A"), resultSet.getString("WRD_NAME"), 
						resultSet.getString("WRD_TELE"),resultSet.getString("WRD_FAX"),
						resultSet.getString("WRD_EMAIL"),resultSet.getInt("WRD_NBEDS"),
						resultSet.getInt("WRD_NQUA_NURS"),resultSet.getInt("WRD_NDOC"),
						resultSet.getBoolean("WRD_IS_PHARMACY"), resultSet.getBoolean("WRD_IS_FEMALE"),
						resultSet.getBoolean("WRD_IS_FREE"),
						resultSet.getInt("WRD_MIN_AGE"),resultSet.getInt("WRD_MAX_AGE"),
	 ...
	 */
	public ArrayList<Ward> getWards() throws OHException {
		ArrayList<Ward> pWard = null;
	...
	/**
	 * Retrieves the {@link Ward} with the specified ward ID.
	 * @param wardID - the ward ID.
	 * @return the retrieved ward.
	 * @throws OHException if an error occurs retrieving the wards.
	 */
	public Ward getWard(String wardID) throws OHException {
		Ward ward = null;
		ArrayList<Object> params = new ArrayList<Object>();
		StringBuilder sqlString = new StringBuilder("SELECT * FROM WARD");
		...
		try {
			ResultSet resultSet = dbQuery.getDataWithParams(sqlString.toString(), params, true);
			while (resultSet.next()) {
				ward = new Ward(resultSet.getString("WRD_ID_A"), resultSet.getString("WRD_NAME"), 
						resultSet.getString("WRD_TELE"),resultSet.getString("WRD_FAX"),
						resultSet.getString("WRD_EMAIL"),resultSet.getInt("WRD_NBEDS"),
						resultSet.getInt("WRD_NQUA_NURS"),resultSet.getInt("WRD_NDOC"),
						resultSet.getBoolean("WRD_IS_PHARMACY"), resultSet.getBoolean("WRD_IS_FEMALE"),
	...
	public boolean newWard(Ward ward) throws OHException {
		...
		try {
			String sqlstring = "INSERT INTO WARD (WRD_ID_A, WRD_NAME, WRD_TELE, WRD_FAX, WRD_EMAIL," +
					" WRD_NBEDS, WRD_NQUA_NURS, WRD_NDOC, WRD_IS_PHARMACY, WRD_IS_FEMALE, WRD_IS_FREE, WRD_MIN_AGE, WRD_MAX_AGE) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
			Object[] objects = {ward.getCode(),ward.getDescription(),ward.getTelephone(),ward.getFax(),ward.getEmail(),ward.getBeds(),ward.getNurs(),ward.getDocs(),ward.isPharmacy()? 1:0,ward.isFemale()? 1:0,ward.isFree()? 1:0,ward.getMinimumMonths(),ward.getMaximumMonths()};
	...
	public boolean updateWard(Ward ward,boolean isConfirmedOverwriteRecord) throws OHException{
		...
		if(isCodePresent(ward.getCode())) {			//ok the record is present, it was not deleted
			try {
				...
				Object[] objects = {ward.getDescription(),ward.getTelephone(),ward.getFax(),ward.getEmail(),ward.getBeds(),ward.getNurs(),ward.getDocs(),ward.isPharmacy()? 1:0,ward.isFemale()? 1:0,ward.isFree()? 1:0,ward.getMinimumMonths(),ward.getMaximumMonths(),ward.getCode()};
